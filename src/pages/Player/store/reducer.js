...
const handleInsertSong = (state, song) => {
  ...
  // 如果是当前歌曲直接不处理
  if (fpIndex === currentIndex && currentIndex !== -1) return false
  currentIndex++
  // 把歌曲放进去，放到当前播放曲目的下一个位置
  playList.splice(currentIndex, 0, song)
  // 如果列表中已经存在要添加的歌，暂且称它 oldsong
  if (fpIndex > -1) {
    // 如果 oldsong 的索引在目前播放歌曲的索引小，那么删除它，同时当前index要减一
  ...
  if (fsIndex > -1) {
    // 跟上面的逻辑类似，如果在前面就删掉，index--；如果在后面就直接删除
